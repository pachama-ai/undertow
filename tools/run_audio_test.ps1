<#
    run_audio_test.ps1 — FOKUSSIERTER Audio-Test (nur tests/audio_tests.lua).

    Baut ein eigenes Test-PDX ausschließlich mit dem Audio-Test und wertet
    TestReport.audio aus. Bewusst KEINE komplette Testsuite — für die
    Isolation des Sounddesign-Ausbaus.
    ...
    Ablauf:
      ...
      4. Projektmodule + tests/audio_tests.lua hineinkopieren.
      5. pdxinfo und Runner-main.lua erzeugen (importiert nur audio_tests).
...
try {
    ...
    $srcDir = Join-Path $buildDir '_asrc'
    $outDir = Join-Path $buildDir '_aout'
    $bundleId = 'com.selina.ringe.testaudio'
    ...
    # --- 4) Dateien kopieren (alle Projektmodule + nur der Audio-Test) -----
    $srcModules = @(
    ...
    $srcTest = Join-Path $root 'tests\audio_tests.lua'
    if (-not (Test-Path -LiteralPath $srcTest)) { throw "audio_tests.lua fehlt: $srcTest" }
    Copy-Item -LiteralPath $srcTest -Destination (Join-Path $srcDir 'audio_tests.lua') -Force

    # --- 5) pdxinfo und Runner-main.lua ------------------------------------
    Set-Content -LiteralPath (Join-Path $srcDir 'pdxinfo') -Value ("name=audioTest`nversion=0.1`nbundleID=" + $bundleId + "`n") -Encoding ascii
    $runner = @'
...
if ok and TestReport.audio then
    print("RESULT passed=" .. TestReport.audio.pass .. " failed=" .. TestReport.audio.fail .. " complete=1")
else
    if not ok then
        print("TESTS_RUN_ERROR: " .. tostring(err))
    else
        print("MISSING_TEST_RESULTS: audio=" .. tostring(TestReport.audio ~= nil))
    end
end
if logFile then
    logFile:flush()
...
'@
    ...
    $pdxOut = Join-Path $outDir 'audioTest.pdx'
    & $pdc $srcDir $pdxOut
    if ($LASTEXITCODE -ne 0) {
        throw "pdc-Build des Audio-TEST-PDX fehlgeschlagen (Exit $LASTEXITCODE)"
    }
