<#
    run_room7_test.ps1 – FOKUSSIERTER Level-7-Test (nur tests/room7_tests.lua).

    Baut ein eigenes Test-PDX ausschließlich mit dem Raum-7-Test und wertet
    TestReport.room7 aus. Bewusst KEINE komplette Testsuite – für die
    ...
    Ablauf:
      ...
      4. Projektmodule + tests/room7_tests.lua hineinkopieren.
      5. pdxinfo und Runner-main.lua erzeugen (importiert nur room7_tests).
...
try {
    ...
    $srcDir = Join-Path $buildDir '_room7_src'
    $outDir = Join-Path $buildDir '_room7_out'
    $bundleId = 'com.selina.ringe.test7'
    ...
    # --- 4) Dateien kopieren (alle Projektmodule + nur der Raum-7-Test) ----
    $srcModules = @(
    ...
    $srcTest = Join-Path $root 'tests\room7_tests.lua'
    if (-not (Test-Path -LiteralPath $srcTest)) { throw "room7_tests.lua fehlt: $srcTest" }
    Copy-Item -LiteralPath $srcTest -Destination (Join-Path $srcDir 'room7_tests.lua') -Force

    # --- 5) pdxinfo und Runner-main.lua ------------------------------------
    Set-Content -LiteralPath (Join-Path $srcDir 'pdxinfo') -Value ("name=Room7Test`nversion=0.1`nbundleID=" + $bundleId + "`n") -Encoding ascii
    $runner = @'
...
if ok and TestReport.room7 then
    print("RESULT passed=" .. TestReport.room7.pass .. " failed=" .. TestReport.room7.fail .. " complete=1")
else
    if not ok then
        print("TESTS_RUN_ERROR: " .. tostring(err))
    else
        print("MISSING_TEST_RESULTS: room7=" .. tostring(TestReport.room7 ~= nil))
    end
end
if logFile then
    logFile:flush()
...
'@
    ...
    $pdxOut = Join-Path $outDir 'Room7Test.pdx'
    & $pdc $srcDir $pdxOut
    if ($LASTEXITCODE -ne 0) {
        throw "pdc-Build des ROOM-7-TEST-PDX fehlgeschlagen (Exit $LASTEXITCODE)"
    }
