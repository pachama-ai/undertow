<#
    run_room5_test.ps1 — FOKUSSIERTER Level-5-Test (nur tests/room5_tests.lua).

    Baut ein eigenes Test-PDX ausschließlich mit dem Raum-5-Test und wertet
    TestReport.room5 aus. Bewusst KEINE komplette Testsuite — für die
    ...
    Ablauf:
      ...
      4. Projektmodule + tests/room5_tests.lua hineinkopieren.
      5. pdxinfo und Runner-main.lua erzeugen (importiert nur room5_tests).
...
try {
    ...
    $srcDir = Join-Path $buildDir '_room5_src'
    $outDir = Join-Path $buildDir '_room5_out'
    $bundleId = 'com.selina.ringe.test5'
    ...
    # --- 4) Dateien kopieren (alle Projektmodule + nur der Raum-5-Test) ----
    $srcModules = @(
    ...
    $srcTest = Join-Path $root 'tests\room5_tests.lua'
    if (-not (Test-Path -LiteralPath $srcTest)) { throw "room5_tests.lua fehlt: $srcTest" }
    Copy-Item -LiteralPath $srcTest -Destination (Join-Path $srcDir 'room5_tests.lua') -Force

    # --- 5) pdxinfo und Runner-main.lua ------------------------------------
    Set-Content -LiteralPath (Join-Path $srcDir 'pdxinfo') -Value ("name=Room5Test`nversion=0.1`nbundleID=" + $bundleId + "`n") -Encoding ascii
    $runner = @'
...
if ok and TestReport.room5 then
    print("RESULT passed=" .. TestReport.room5.pass .. " failed=" .. TestReport.room5.fail .. " complete=1")
else
    if not ok then
        print("TESTS_RUN_ERROR: " .. tostring(err))
    else
        print("MISSING_TEST_RESULTS: room5=" .. tostring(TestReport.room5 ~= nil))
    end
end
if logFile then
    logFile:flush()
...
'@
    ...
    $pdxOut = Join-Path $outDir 'Room5Test.pdx'
    & $pdc $srcDir $pdxOut
    if ($LASTEXITCODE -ne 0) {
        throw "pdc-Build des Room-5-Test-PDX fehlgeschlagen (Exit $LASTEXITCODE)"
    }
