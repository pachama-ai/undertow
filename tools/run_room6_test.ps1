<#
    run_room6_test.ps1 ??? FOKUSSIERTER Level-6-Test (nur tests/room6_tests.lua).

    Baut ein eigenes Test-PDX ausschlie??lich mit dem Raum-6-Test und wertet
    TestReport.room6 aus. Bewusst KEINE komplette Testsuite ??? f??r die
    ...
    Ablauf:
      ...
      4. Projektmodule + tests/room6_tests.lua hineinkopieren.
      5. pdxinfo und Runner-main.lua erzeugen (importiert nur room6_tests).
...
try {
    ...
    $srcDir = Join-Path $buildDir '_room6_src'
    $outDir = Join-Path $buildDir '_room6_out'
    $bundleId = 'com.selina.ringe.test6'
    ...
    # --- 4) Dateien kopieren (alle Projektmodule + nur der Raum-6-Test) ----
    $srcModules = @(
    ...
    $srcTest = Join-Path $root 'tests\room6_tests.lua'
    if (-not (Test-Path -LiteralPath $srcTest)) { throw "room6_tests.lua fehlt: $srcTest" }
    Copy-Item -LiteralPath $srcTest -Destination (Join-Path $srcDir 'room6_tests.lua') -Force

    # --- 5) pdxinfo und Runner-main.lua ------------------------------------
    Set-Content -LiteralPath (Join-Path $srcDir 'pdxinfo') -Value ("name=Room6Test`nversion=0.1`nbundleID=" + $bundleId + "`n") -Encoding ascii
    $runner = @'
...
if ok and TestReport.room6 then
    print("RESULT passed=" .. TestReport.room6.pass .. " failed=" .. TestReport.room6.fail .. " complete=1")
else
    if not ok then
        print("TESTS_RUN_ERROR: " .. tostring(err))
    else
        print("MISSING_TEST_RESULTS: room6=" .. tostring(TestReport.room6 ~= nil))
    end
end
if logFile then
    logFile:flush()
...
'@
    ...
    $pdxOut = Join-Path $outDir 'Room6Test.pdx'
    & $pdc $srcDir $pdxOut
    if ($LASTEXITCODE -ne 0) {
        throw "pdc-Build des ROOM-6-TEST-PDX fehlgeschlagen (Exit $LASTEXITCODE)"
    }
