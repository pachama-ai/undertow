<#
    run_room3_test.ps1 ??? FOKUSSIERTER Level-4-Test (nur tests/room3_tests.lua).

    Baut ein eigenes Test-PDX ausschlie??lich mit dem Raum-4-Test und wertet
    TestReport.room3 aus. Bewusst KEINE komplette Testsuite ??? f??r die
    ...
    Ablauf:
      ...
      4. Projektmodule + tests/room3_tests.lua hineinkopieren.
      5. pdxinfo und Runner-main.lua erzeugen (importiert nur room3_tests).
...
try {
    ...
    $srcDir = Join-Path $buildDir '_room3_src'
    $outDir = Join-Path $buildDir '_room3_out'
    $bundleId = 'com.selina.ringe.test3'
    ...
    $srcTest = Join-Path $root 'tests\room3_tests.lua'
    if (-not (Test-Path -LiteralPath $srcTest)) { throw "room3_tests.lua fehlt: $srcTest" }
    Copy-Item -LiteralPath $srcTest -Destination (Join-Path $srcDir 'room3_tests.lua') -Force

    # --- 5) pdxinfo und Runner-main.lua ------------------------------------
    Set-Content -LiteralPath (Join-Path $srcDir 'pdxinfo') -Value ("name=room3Test`nversion=0.1`nbundleID=" + $bundleId + "`n") -Encoding ascii
    $runner = @'
...
if ok and TestReport.room3 then
    print("RESULT passed=" .. TestReport.room3.pass .. " failed=" .. TestReport.room3.fail .. " complete=1")
else
    if not ok then
        print("TESTS_RUN_ERROR: " .. tostring(err))
    else
        print("MISSING_TEST_RESULTS: room3=" .. tostring(TestReport.room3 ~= nil))
    end
end
if logFile then
    logFile:flush()
...
'@
    ...
    $pdxOut = Join-Path $outDir 'room3Test.pdx'
    & $pdc $srcDir $pdxOut
    if ($LASTEXITCODE -ne 0) {
        throw "pdc-Build des Room-3-TEST-PDX fehlgeschlagen (Exit $LASTEXITCODE)"
    }
