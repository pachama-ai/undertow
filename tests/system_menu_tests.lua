...
-- Teil C: „Zum Menü"-Contract — Menu.show() setzt Weiter-Initialauswahl,
--   entfernt Gameplay-Systemitems, schreibt/liest Datastore nicht; Weiter
--   danach startet frisch; Von vorn danach startet Raum 1 mit unverändertem Save.
--
...
-- Raum 1: verändern (Player bewegen, Schalter, Undo) -> Reset -> frisch.
State.init(Levels[1])
Undo.clear()
Room.init()
Room.movePlayer(45) -- bewegt Player und löst ggf. S1
State.setSwitch("S1", "A") -- Schalter verändern
Undo.push(State.snapshot()) -- Undo-Snapshot
State.init(Levels[2])
Undo.clear()
Room.init()
Room.movePlayer(45)
...
-- Räume 3-6: analog (Punkt 57/48, Abschlussphase A: Restart in allen Räumen).
resetRoomContract(3)
...
-- wieder aktiv (Core-Timer neu; nächster Puls bei coreFrequency(6), Raum 6 ist
-- der Finalraum).
resetRoomContract(6)
Audio.setCompleted()
...
-- Menu.show() setzt die Initialauswahl auf „Weiter" (Punkt 30).
readCount = 0
writeCount = 0
Menu.show()
check(Menu.isActive() == true, "sm-menu: Menu aktiv")
check(Menu.getSelectedIndex() == 1, "sm-menu: Weiter initial ausgewählt (Punkt 30)")
...
-- „Von vorn" danach bei Save=6: Raum 1 frisch, Save bleibt 6, kein Write
-- (Punkt 28/67/18).
resetRoomContract(1)
assertFreshRoom(1, "sm-menu-vonvorn:")
check(writeCount == 0, "sm-menu-vonvorn: kein Datastore-Write (Save bleibt 6)")
