-- Tests für source/ui/transition.lua (Abschlussphase B): Outro nach Raum 6.
...
-- --- 2) Geometrie-Erfassung (Raum 6) ------------------------------------------
Camera.init(2) -- Room-6-outer (Finalraum)
Transition.reset()
...
check(Transition.outerR == Config.outerRadius, "tr: outerR = 104 (Camera Room-6)")
check(Transition.innerR == Config.innerRadius, "tr: innerR = 68 (Camera Room-6)")
check(Transition.coreStartR == Config.coreRadius + 5 * Config.coreGrowthPerRoom,
    "tr: coreStartR = Room-6-Kern (60)")
