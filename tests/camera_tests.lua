...
-- Echte Ringnummern aus den Leveldaten (Räume 1-7).
...
local R7_OUTER <const> = Levels[7].rings.outer -- 1
local R7_INNER <const> = Levels[7].rings.inner -- 0
...
-- --- Pflicht-Test: 6 -> 7 (Abschlussphase A, neuer Finalraum) ---------------
do
    Camera.init(R6_OUTER)
    Camera.beginRoomTransition(R6_OUTER, R6_INNER, R7_OUTER, R7_INNER)
    Camera.update(0.3)
    check(approx(Camera.getProgress(), 0.25), "6->7: raw 0.25 bei 0,3 s")
    Camera.update(0.9)
    check(Camera.getCurrentOuterRing() == R7_OUTER, "6->7: Zielring 1")
    check(approx(Camera.getRadius(R6_OUTER), Config.outerRadius + ringSpacing), "6->7: alter outer 140")
    check(Camera.getRadius(R7_OUTER) == Config.outerRadius, "6->7: neuer outer 104")
    check(Camera.getRadius(R7_INNER) == Config.innerRadius, "6->7: neuer inner 68")
end
...
-- --- Pflicht-Test: Raum 7 stabil (Finalraum, keine Transition ohne Gate) ----
do
    Camera.init(R7_OUTER) -- Raum 7: outer 1, inner 0
    check(Camera.isTransitioning() == false, "raum7: keine Transition")
    check(Camera.getRadius(R7_OUTER) == Config.outerRadius, "raum7: outer 104")
    check(Camera.getRadius(R7_INNER) == Config.innerRadius, "raum7: inner 68")
    check(approx(Camera.getRadius(R7_OUTER + 1), Config.outerRadius + ringSpacing), "raum7: vorheriger outer 140")
    check(Camera.getTargetOuterRing() == R7_OUTER, "raum7: Ziel stabil (Room7-Completion lädt nichts)")
end
