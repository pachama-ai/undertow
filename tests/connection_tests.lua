...
-- --- Test 9: Gate aktiv auf inner (Kernbrücke = normale Brücke) ----------
do
    setup(makeGateRoom())
    local result = Room.tryUseConnection()
    ...
    check(result.roomComplete == false and result.crossing == true,
        "gate aktiv: Kernbrücken-Transit (Abschluss erst nach Transit)")
    check(Bridge.isCrossing() == true, "gate aktiv: Transit aktiv")
    local done, shared, _, center = Bridge.update(Config.bridgeAnimDuration + 0.001)
    check(done == true and center == true, "gate aktiv: Center-Transit abgeschlossen")
    check(State.room.name == "Gate", "gate aktiv: kein Raumwechsel")
    check(State.player.ring == "inner", "gate aktiv: Ring bleibt Gate-Ring")
    check(State.player.angle == 0, "gate aktiv: Winkel auf Achse")
...
-- --- Test 17: Segment vor der Brücke (auf dem Zielring) blockiert ---------
-- NEUE REGEL: Überdeckt eine GESCHLOSSENE Blende die Brückenachse auf einem
-- der beiden Ringe (auch auf dem Zielring), ist die Brücke NICHT nutzbar —
-- der Player kann sie nicht überqueren (auch kein Landen im geschlossenen
-- Segment).
do
    setup(makeG7TransitRoom()) -- S1=A -> B1 aktiv, D1 (inner@90) geschlossen
    check(Bridge.isUsable(State.room.bridges[1], 90) == false,
        "segment-zielring: Brücke NICHT nutzbar (D1 inner@90 geschlossen)")
    local result = Room.tryUseConnection()
    check(result.used == false, "segment-zielring: A startet KEINEN Transit")
    check(State.player.ring == "outer", "segment-zielring: Player bleibt auf outer")
end

-- --- Test 18: geschlossene Blende auf dem Quellring (Segment vor Brücke) ---
-- NEUE REGEL: Überdeckt eine GESCHLOSSENE Blende auf dem Ring des Spielers
-- die Brückenachse („Segment genau vor der Brücke“), ist die Brücke NICHT
-- nutzbar — der Player kann sie nicht überqueren, solange das Segment zu ist.
do
    setup(makeOtherRingRoom()) -- S1=A -> B1 aktiv, D1 (outer@90) geschlossen
    Room.syncPhysicalShutters()
    check(Bridge.isUsable(State.room.bridges[1], 90) == false,
        "segment-block: Brücke NICHT nutzbar (Segment vor ihr, Quellring)")
    local result = Room.tryUseConnection()
    check(result.used == false, "segment-block: A löst KEINEN Transit aus")
    check(State.player.ring == "outer", "segment-block: Player bleibt auf outer")
    check(State.elementStates["D1"] == false,
        "segment-block: D1 logisch geschlossen (G7 pendingClose, da Player im Bogen)")
end
...
-- --- Test 34: Segment vor der Brücke auf dem Zielring (regelbestätigt) ----
do
    setup(makeG7TransitRoom()) -- S1=A -> B1 aktiv, D1 (inner@90) geschlossen
    local res = Room.tryUseConnection()
    check(res.used == false, "segment-zielring2: KEIN Transit (D1 inner@90 zu)")
    check(Bridge.isCrossing() == false, "segment-zielring2: kein aktiver Transit")
    check(State.player.ring == "outer", "segment-zielring2: Player bleibt auf outer")
end
...
do
    ...
    check(result.crossing == true and result.roomComplete == false,
        "gate wraparound: Kernbrücken-Transit (Abschluss nach Transit)")
    local wdone, _, _, wcenter = Bridge.update(Config.bridgeAnimDuration + 0.001)
    check(wdone == true and wcenter == true, "gate wraparound: Center-Transit abgeschlossen")
end
...
do
    ...
    local result = Room.tryUseConnection()
    check(result.used == true and result.crossing == true, "gate zustand: genutzt (Kernbrücken-Transit)")
    ...
    check(State.player.angle == angleBefore, "gate zustand: Winkel unverändert (Achse = Dock)")
    Bridge.update(Config.bridgeAnimDuration + 0.001)
    check(Bridge.isCrossing() == false, "gate zustand: Transit abgeschlossen")
end
...
do
    ...
    check(result.crossing == true and result.roomComplete == false,
        "gate undo-vorhanden: Kernbrücken-Transit (Abschluss nach Transit)")
    Bridge.update(Config.bridgeAnimDuration + 0.001)
...
do
    ...
    for _, room in ipairs(Levels) do
        -- Alle Ringe des Raums (2-Ring-Standard: outer/inner; 3-Ring-Raum
        -- Level 4 zusätzlich middle). Brücken sind NUR auf ihren verbundenen
        -- Ringen Kandidaten (b.rings = { <RingA>, <RingB> }; Legacy ohne
        -- rings-Feld = auf beiden Ringen). Das Tor auf seinem Gate-Ring.
        local rings = { "outer" }
        if room.rings.middle ~= nil then rings[#rings + 1] = "middle" end
        rings[#rings + 1] = "inner"
        for _, ring in ipairs(rings) do
            local conns = {}
            for _, b in ipairs(room.bridges) do
                local onRing
                if b.rings and #b.rings == 2 then
                    onRing = (b.rings[1] == ring or b.rings[2] == ring)
                else
                    onRing = (ring == "outer" or ring == "inner")
                end
                if onRing then
                    conns[#conns + 1] = { id = b.id, angle = b.angle }
                end
            end
            if room.gate and ring == (room.gate.ring or "inner") then
...
-- --- Einmal-Brücke (oneShot): kollabiert nach der ersten Benutzung ---------
do
    local room = {
        name = "EinmalBruecke",
        rings = { outer = 7, inner = 6 },
    ...
    setup(room)
    State.setSwitch("S1", "A") -- B1 aktiv
    check(State.elementStates["B1"] == true, "oneshot-br: B1 aktiv (S1=A)")
    check(State.consumedBridges["B1"] == nil, "oneshot-br: unbenutzt")
    check(Bridge.isUsable(room.bridges[1], 90) == true, "oneshot-br: nutzbar vor Benutzung")

    -- Erste Benutzung: die Brücke bleibt WÄHREND der Überquerung stehen und
    -- kollabiert erst, wenn der Player über sie hinweg ist (Transitabschluss).
    local res = Room.tryUseConnection()
    check(res.used == true and res.kind == "bridge" and res.id == "B1",
        "oneshot-br: erster Übergang genutzt")
    check(State.consumedBridges["B1"] == nil, "oneshot-br: beim Start noch nicht verbraucht")
    check(State.elementStates["B1"] == true, "oneshot-br: B1 bleibt während des Transits aktiv")
    check(Bridge.isCrossing() == true, "oneshot-br: Transit läuft")
    -- Halbe Überquerung: Brücke steht noch (nicht kollabiert).
    Bridge.update(Config.bridgeAnimDuration / 2)
    check(State.consumedBridges["B1"] == nil, "oneshot-br: nach halbem Transit noch nicht verbraucht")
    -- S1 steht weiterhin auf A, aber nach dem Kollaps bleibt B1 dauerhaft
    -- inaktiv (Kollaps überstimmt den Schalter).
    check(State.switchStates["S1"] == "A", "oneshot-br: S1 weiterhin A")
    completeTransit()
    check(State.player.ring == "inner", "oneshot-br: Transit abgeschlossen (inner)")
    check(State.consumedBridges["B1"] == true, "oneshot-br: erst NACH der Überquerung verbraucht")
    check(State.elementStates["B1"] == false, "oneshot-br: elementState false (kollabiert)")

    -- Nach dem Kollaps nicht mehr nutzbar.
    State.player.ring = "outer"
    State.player.angle = 90
    check(Bridge.isUsable(room.bridges[1], 90) == false, "oneshot-br: nicht mehr nutzbar")
    local res2 = Room.tryUseConnection()
    check(res2.used == false, "oneshot-br: kein zweiter Übergang")
end

-- --- Test 17: Kernbrücke (Gate) = normale Brücke zum Mittelpunkt ----------
-- Center-Bridge-Fix: das Gate ist KEIN eigener Brückentyp — der Transit zum
-- Mittelpunkt nutzt exakt dieselbe Bridge-Logik (gleiche Dauer, gleiche
-- Achse, gleicher Progress), nur das Ziel ist der Kernrand statt eines Rings.
do
    local room = {
        name = "CenterSolo",
        rings = { outer = 7, inner = 6 },
        start = { ring = "inner", angle = 30 },
        switches = {},
        shutters = {},
        bridges = {},
        gate = { id = "T", angle = 30, free = true },
    }
    setup(room)
    check(Gate.isUsable(room.gate, "inner", 30) == true, "center-solo: Tor nutzbar")
    local started = Bridge.beginTransit(room.gate, "inner", "center")
    check(started == true, "center-solo: Center-Transit gestartet")
    check(Bridge.getTransit().toRing == "center", "center-solo: Ziel = center")
    check(Bridge.getTransit().angle == 30, "center-solo: Achse = Torwinkel")
    check(Bridge.getTransit().duration == Config.bridgeAnimDuration, "center-solo: gleiche Dauer wie Ring->Ring")
    check(State.player.angle == 30, "center-solo: Player auf Achse ausgerichtet")
    -- Halber Transit: immer noch aktiv, Player im Zustand (kein Ringwechsel).
    local done1, _, _, center1 = Bridge.update(Config.bridgeAnimDuration / 2)
    check(done1 == false, "center-solo: halber Transit nicht abgeschlossen")
    check(Bridge.isCrossing() == true, "center-solo: Transit noch aktiv")
    check(State.player.ring == "inner", "center-solo: Ring während Transit unverändert")
    -- Abschluss: wasCenter=true, Player bleibt auf dem Gate-Ring (kein Center-Ring).
    local done, wasShared, babyLanded, wasCenter = Bridge.update(Config.bridgeAnimDuration / 2 + 0.001)
    check(done == true and wasCenter == true, "center-solo: abgeschlossen (Center)")
    check(not wasShared, "center-solo: Solo (kein Baby)")
    check(State.player.ring == "inner", "center-solo: Ring bleibt inner")
    check(State.player.angle == 30, "center-solo: Winkel auf Achse")
    check(Undo.count() == 0, "center-solo: kein Undo")
end

-- --- Test 18: GEMEINSAMER Kernbrücken-Transit (Gate mit Baby) ------------
-- Der Ausgang verlangt das Baby (Gate.isUsable); Room.tryUseConnection
-- startet dann den GEMEINSAMEN Center-Transit (Baby voraus, Player folgt).
do
    local room = {
        name = "CenterShared",
        rings = { outer = 7, inner = 6 },
        start = { ring = "inner", angle = 0 },
        baby = { start = { ring = "inner", angle = 5 } },
        switches = {},
        shutters = {},
        bridges = {},
        gate = { id = "T", angle = 0, free = true },
    ...
    setup(room)
    -- Baby auf den Gate-Ring setzen (gemeinsamer Ausgang nötig).
    State.baby.ring = "inner"
    State.baby.angle = 0
    check(Gate.isUsable(room.gate, "inner", 0) == true, "center-shared: Tor nutzbar (Player+Baby)")
    local res = Room.tryUseConnection()
    check(res.used == true and res.kind == "gate" and res.crossing == true and res.roomComplete == false,
        "center-shared: Gate -> gemeinsamer Kernbrücken-Transit")
    check(Bridge.getTransit() ~= nil and Bridge.getTransit().shared == true,
        "center-shared: Shared-Transit aktiv")
    check(Bridge.getTransit().toRing == "center", "center-shared: Ziel = center")
    local total = Config.sharedBridgeHold + Config.sharedBabyLead + Config.sharedPlayerDuration
    local done, wasShared, babyLanded, wasCenter = Bridge.update(total + 0.001)
    check(done == true and wasShared == true and wasCenter == true,
        "center-shared: gemeinsamer Center-Transit abgeschlossen")
    check(State.player.ring == "inner" and State.baby.ring == "inner",
        "center-shared: beide am Gate-Ring (Ziel = Mittelpunkt)")
    check(State.player.angle == 0, "center-shared: Player auf Achse")
    check(Undo.count() == 0, "center-shared: kein Undo")
end

-- --- Test 24: Segment genau vor der Brücke (offen -> nutzbar) ------------
-- Die neue Regel vollständig: Solange ein geschlossenes Segment die freie
-- Brückenachse überdeckt, ist die Brücke nicht nutzbar (auch im Dock-Bereich);
-- sobald das Segment öffnet, wird die Brücke sofort nutzbar.
do
    local room = {
        name = "SegmentVorBruecke",
        rings = { outer = 7, inner = 6 },
        start = { ring = "outer", angle = 90 },
        switches = {
            { id="S1", ring="outer", angle=45, symbol=1, onA="SH", onB={}, state="B" },
        },
        shutters = {
            { id="SH", ring="outer", angle=90 },
        },
        bridges = {
            { id="B0", angle=90, free=true },
        },
    ...
    setup(room)
    Room.syncPhysicalShutters()
    check(State.elementStates["SH"] == false, "segment-offen: SH geschlossen (Start)")
    check(Bridge.isUsable(room.bridges[1], 90) == false,
        "segment-offen: freie Brücke NICHT nutzbar, solange Segment zu ist")
    local res = Room.tryUseConnection()
    check(res.used == false, "segment-offen: A startet KEINEN Transit (Segment zu)")
    -- Segment öffnen (S1=A -> SH offen): Brücke wird nutzbar.
    State.setSwitch("S1", "A")
    State.deriveElements()
    Room.syncPhysicalShutters()
    check(State.elementStates["SH"] == true, "segment-offen: SH offen (S1=A)")
    check(Bridge.isUsable(room.bridges[1], 90) == true,
        "segment-offen: Brücke nutzbar, sobald Segment offen")
    local res2 = Room.tryUseConnection()
    check(res2.used == true and res2.kind == "bridge",
        "segment-offen: A startet den Transit")
    completeTransit()
    check(State.player.ring == "inner", "segment-offen: Spieler auf inner")
end
