...
-- Skalierung ALLER Ringradien für die Level-Restart-Animation (1 = normal,
-- 0 = alles im Mittelpunkt). nil = keine Skalierung. Rein visuell: der Kern
-- (fixer Mittelpunktskreis) wird NICHT skaliert und bleibt beim Kollaps
-- sichtbar. Die Levelobjekte laufen über die Ringradien automatisch mit.
Camera.restartScale = nil

-- Uniformer Zoom für die Start-Reveal-Animation (Titel -> Level): alle Radien
-- (Ringbahnen UND Kern) werden gemeinsam um (200,120) skaliert, von einem
-- großen Wert (Core = gefüllte Menü-Scheibe) bis 1.0 (Normalgröße). nil =
-- keine Skalierung. Rein visuell; nur während appMode "menu" (Reveal-Phase)
-- aktiv, kein Gameplay-Effekt. Wird NIE zusammen mit restartScale benutzt.
Camera.revealScale = nil
...
-- Während der Level-Restart-Animation skaliert restartScale alle Radien
-- gemeinsam (Kollaps zum Mittelpunkt / Wiederaufbau). Der Radius wird auf ein
-- winziges Minimum geklemmt (nie exakt 0): rein visuell unsichtbar unter dem
-- Kern, aber Divisionen durch 0 (z. B. Shutter-Überschwing) bleiben sicher.
-- Während der Start-Reveal-Animation skaliert revealScale zusätzlich alle
-- Radien gemeinsam (Zoom aus dem Mittelpunkt, s0 -> 1).
function Camera.getRadius(ringNumber)
    local visualOuter = Camera.getVisualOuterRing()
    local radius = config.outerRadius - (visualOuter - ringNumber) * ringSpacing
    if Camera.restartScale ~= nil then
        radius = radius * math.max(Camera.restartScale, 0.001)
    end
    if Camera.revealScale ~= nil then
        radius = radius * Camera.revealScale
    end
    return radius
end

-- Bildschirmradius einer RingNUMMER bei einem gegebenen Fortschritt der
-- Kamera-Transition (0 = Transitionsstart, 1 = Transitionsende). Wird für
-- die kontinuierliche Figuren-Transition benutzt: Start-/Zielradius einer
-- Figur liegen EXAKT auf der Ringgeometrie (kein Sprung, kein Versatz).
-- Ohne laufende Transition = aktueller Radius.
function Camera.getRadiusAtProgress(ringNumber, progress)
    local visualOuter
    local t = Camera.transition
    if t and t.active then
        local eased = smoothstep(math.max(0, math.min(1, progress or 0)))
        visualOuter = t.fromOuterRing + (t.toOuterRing - t.fromOuterRing) * eased
    else
        visualOuter = Camera.currentOuterRing
    end
    return config.outerRadius - (visualOuter - ringNumber) * ringSpacing
end

-- Setzt die Restart-Skalierung (0 = kollabiert, 1 = normal). Rein visuell.
function Camera.setRestartScale(scale)
    Camera.restartScale = scale
end

-- Beendet die Restart-Skalierung (zurück zu normal).
function Camera.clearRestartScale()
    Camera.restartScale = nil
end

-- Setzt die Reveal-Skalierung (s0 > 1 = stark vergrößert, 1 = normal).
-- Rein visuell; nur während der Start-Reveal-Phase aktiv.
function Camera.setRevealScale(scale)
    Camera.revealScale = scale
end

-- Beendet die Reveal-Skalierung (zurück zu normal).
function Camera.clearRevealScale()
    Camera.revealScale = nil
end
...
-- duration (optional, Sekunden): Dauer der Interpolation; Default ist
-- Config.cameraDuration. Der radiale Raumübergang nutzt eine eigene, kürzere
-- Dauer (Config.roomTransitionDuration); der Standard bleibt unverändert.
-- Rückgabe: true bei Erfolg, false wenn bereits eine Transition läuft.
function Camera.beginRoomTransition(fromOuterRing, fromInnerRing, toOuterRing, toInnerRing, hold, duration)
    if Camera.isTransitioning() then
        return false
    end
    ...
    Camera.transition = {
        ...
        elapsed = 0,
        hold = hold or 0,
        duration = duration or config.cameraDuration,
    }
    return true
end
